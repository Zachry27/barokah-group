...
import { CalendarDays, Image as ImageIcon, LayoutDashboard, LogOut, Megaphone, Package, Plus, Quote, Save, ShieldCheck, Sparkles } from 'lucide-react';
...
export default function AdminPage() {
  ...
  async function refresh() {
    ...
    setAnnouncement(value?.text || '');
    setBaggage((bag.data || []) as Row[]);
    setProducts((prod.data || []) as Row[]);
    setBrochures((brochure.data || []) as Row[]);
    setTestimonials((test.data || []) as Row[]);
  }
  ...
  async function authSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const fd = new FormData(e.currentTarget); const email = String(fd.get('email') || '').toLowerCase().trim(); const password = String(fd.get('password') || '');
    if (email !== ADMIN_EMAIL) { setNotice('Email ini tidak memiliki akses admin.'); return; }
    const login = await supabase.auth.signInWithPassword({ email, password });
    if (!login.error) return;
    const signup = await supabase.auth.signUp({ email, password });
    if (signup.error) setNotice(signup.error.message); else setNotice('Akun dibuat. Cek email konfirmasi Supabase jika diminta, lalu login kembali.');
  }

  async function saveAnnouncement() {
    const { error } = await supabase.from('site_settings').upsert({ key:'announcement', value:{ text: announcement, active:true } }, { onConflict:'key' });
    setNotice(error ? error.message : 'Pengumuman berhasil disimpan.');
  }
  async function saveRow(table: string, row: Row) {
    const { error } = await supabase.from(table).update(row).eq('id', row.id);
    setNotice(error ? error.message : 'Perubahan berhasil disimpan.');
    if (!error) await refresh();
  }
  async function addRow(table: string, defaults: Row) {
    const { error } = await supabase.from(table).insert(defaults);
    setNotice(error ? error.message : 'Item baru berhasil ditambahkan.');
    if (!error) await refresh();
  }
  ...
  const menu: {id:Tab;label:string;icon:typeof LayoutDashboard}[] = [
    {id:'overview',label:'Overview',icon:LayoutDashboard},{id:'announcement',label:'Pengumuman',icon:Megaphone},{id:'baggage',label:'Jadwal Bagasi',icon:CalendarDays},{id:'products',label:'Produk & Promo',icon:Package},{id:'brochures',label:'Brosur',icon:ImageIcon},{id:'testimonials',label:'Testimoni',icon:Quote}
  ];
  ...
  {tab==='brochures'&&<><AddButton onClick={()=>addRow('brochures',{title:'Brosur Baru',image_url:'/brochures/ringkasan-layanan.jpg',alt_text:'Brosur Barokah Group',sort_order:brochures.length+1,is_active:true})} label="Tambah brosur"/><EditableRows rows={brochures} fields={['title','image_url','alt_text','sort_order','is_active']} onSave={row=>saveRow('brochures',row)}/></>}
  {tab==='testimonials'&&<><AddButton onClick={()=>addRow('testimonials',{name:'Nama Pelanggan',role:'Pelanggan Barokah Group',quote:'Tulis pengalaman pelanggan di sini.',rating:5,sort_order:testimonials.length+1,is_active:true})} label="Tambah testimoni"/><EditableRows rows={testimonials} fields={['name','role','quote','rating','sort_order','is_active']} onSave={row=>saveRow('testimonials',row)}/></>}
